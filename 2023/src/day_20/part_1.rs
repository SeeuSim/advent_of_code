...
fn get_cycles(lines: &Vec<String>) -> usize {
    ...
    for v in module_map.clone().values().filter(|x| x.mod_type == ModuleType::FlipFlop) {
    ...
    loop {
        n_it += 1;
        let mut queue = VecDeque::from_iter(initial_targets.iter().map(|x| 
        ...
        while !queue.is_empty() {
            ...
            match module.mod_type {
                ModuleType::FlipFlop => {
                    ...
                    module.outputs.iter().for_each(|x| {
                        queue.push_back((module.label, *x, is_out_high))
                    });
                },
    ...
    let prelim_vals = (0..excluded).map(|idx| {
        acc[idx]
    }).fold((0, 0), |acc, e| {
        (acc.0 + e.0, acc.1 + e.1)
    });

    let vals_per_cycle = acc.iter().fold((0, 0), |acc, e| {
        (acc.0 + e.0, acc.1 + e.1)
    });
...
pub fn placeholder(file_name: &String) {
    ...
    assert!(test_ans == 32000000, "Test Case 1: Expected 32000000, Got {test_ans}");
    ...
    assert!(test_ans == 11687500, "Test Case 2: Expected 11687500, Got {test_ans}");
